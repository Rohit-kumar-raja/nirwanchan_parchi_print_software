<form class="text-center row clinic-button" id="edit{{ $item->id }}"
    action="{{ route($route . '.destroy', $item->id) }}">
    <button type="button" class="btn btn-warning btn-sm tooltip1"
        onclick="editForm('{{ route($route . '.edit', $item->id) }}', 'edit_form')" data-toggle="modal"
        data-target="#edit">
        <i class="fas fa-edit"></i> <span> Edit {{ $page }} </span>
    </button>

    <button type="button" class="btn btn-warning btn-sm tooltip1"
        onclick="editForm('{{ route($route . '.show', $item->id) }}', 'show_form')" data-toggle="modal"
        data-target="#show">
    ...
    <button type="button" class="btn btn-warning btn-sm tooltip1"
        ...
        data-toggle="modal" data-target="#password_change">
